<template>
    <div class="self">
        <div class="main">
            ...
            <input type="text" class="line" :placeholder="useUserStore.userInfo!.line===''?'请输入个性签名':''" v-if="editLine" @keydown.enter="editLineFn" :value="useUserStore.userInfo!.line">
            <div class="line" @dblclick="editLine=true" v-else>{{useUserStore.userInfo!.line===''?'暂未设置':useUserStore.userInfo!.line}}</div>
            <div class="phone">
                <svg class="icon">
                    <use xlink:href="#icon-shouji"></use>
                </svg>
                <input type="text" :placeholder="useUserStore.userInfo!.phone ===''?'请输入手机号':''" v-if="editPhone" @keydown.enter="editPhoneFn" :value="useUserStore.userInfo!.phone">
