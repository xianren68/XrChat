/**
 * Generated by the protoc-gen-ts.  DO NOT EDIT!
 * compiler version: 5.26.0
 * source: option.proto
 * git: https://github.com/thesayyn/protoc-gen-ts */
import * as pb_1 from 'google-protobuf'
export namespace option {
	export class updateLine extends pb_1.Message {
		#one_of_decls: number[][] = []
		constructor(
			data?:
				| any[]
				| {
						id?: number
						line?: string
				  }
		) {
			super()
			pb_1.Message.initialize(this, Array.isArray(data) ? data : [], 0, -1, [], this.#one_of_decls)
			if (!Array.isArray(data) && typeof data == 'object') {
				if ('id' in data && data.id != undefined) {
					this.id = data.id
				}
				if ('line' in data && data.line != undefined) {
					this.line = data.line
				}
			}
		}
		get id() {
			return pb_1.Message.getFieldWithDefault(this, 1, 0) as number
		}
		set id(value: number) {
			pb_1.Message.setField(this, 1, value)
		}
		get line() {
			return pb_1.Message.getFieldWithDefault(this, 2, '') as string
		}
		set line(value: string) {
			pb_1.Message.setField(this, 2, value)
		}
		static fromObject(data: { id?: number; line?: string }): updateLine {
			const message = new updateLine({})
			if (data.id != null) {
				message.id = data.id
			}
			if (data.line != null) {
				message.line = data.line
			}
			return message
		}
		toObject() {
			const data: {
				id?: number
				line?: string
			} = {}
			if (this.id != null) {
				data.id = this.id
			}
			if (this.line != null) {
				data.line = this.line
			}
			return data
		}
		serialize(): Uint8Array
		serialize(w: pb_1.BinaryWriter): void
		serialize(w?: pb_1.BinaryWriter): Uint8Array | void {
			const writer = w || new pb_1.BinaryWriter()
			if (this.id != 0) writer.writeUint64(1, this.id)
			if (this.line.length) writer.writeString(2, this.line)
			if (!w) return writer.getResultBuffer()
		}
		static deserialize(bytes: Uint8Array | pb_1.BinaryReader): updateLine {
			const reader = bytes instanceof pb_1.BinaryReader ? bytes : new pb_1.BinaryReader(bytes),
				message = new updateLine()
			while (reader.nextField()) {
				if (reader.isEndGroup()) break
				switch (reader.getFieldNumber()) {
					case 1:
						message.id = reader.readUint64()
						break
					case 2:
						message.line = reader.readString()
						break
					default:
						reader.skipField()
				}
			}
			return message
		}
		serializeBinary(): Uint8Array {
			return this.serialize()
		}
		static deserializeBinary(bytes: Uint8Array): updateLine {
			return updateLine.deserialize(bytes)
		}
	}
	export class updateName extends pb_1.Message {
		#one_of_decls: number[][] = []
		constructor(
			data?:
				| any[]
				| {
						id?: number
						name?: string
				  }
		) {
			super()
			pb_1.Message.initialize(this, Array.isArray(data) ? data : [], 0, -1, [], this.#one_of_decls)
			if (!Array.isArray(data) && typeof data == 'object') {
				if ('id' in data && data.id != undefined) {
					this.id = data.id
				}
				if ('name' in data && data.name != undefined) {
					this.name = data.name
				}
			}
		}
		get id() {
			return pb_1.Message.getFieldWithDefault(this, 1, 0) as number
		}
		set id(value: number) {
			pb_1.Message.setField(this, 1, value)
		}
		get name() {
			return pb_1.Message.getFieldWithDefault(this, 2, '') as string
		}
		set name(value: string) {
			pb_1.Message.setField(this, 2, value)
		}
		static fromObject(data: { id?: number; name?: string }): updateName {
			const message = new updateName({})
			if (data.id != null) {
				message.id = data.id
			}
			if (data.name != null) {
				message.name = data.name
			}
			return message
		}
		toObject() {
			const data: {
				id?: number
				name?: string
			} = {}
			if (this.id != null) {
				data.id = this.id
			}
			if (this.name != null) {
				data.name = this.name
			}
			return data
		}
		serialize(): Uint8Array
		serialize(w: pb_1.BinaryWriter): void
		serialize(w?: pb_1.BinaryWriter): Uint8Array | void {
			const writer = w || new pb_1.BinaryWriter()
			if (this.id != 0) writer.writeUint64(1, this.id)
			if (this.name.length) writer.writeString(2, this.name)
			if (!w) return writer.getResultBuffer()
		}
		static deserialize(bytes: Uint8Array | pb_1.BinaryReader): updateName {
			const reader = bytes instanceof pb_1.BinaryReader ? bytes : new pb_1.BinaryReader(bytes),
				message = new updateName()
			while (reader.nextField()) {
				if (reader.isEndGroup()) break
				switch (reader.getFieldNumber()) {
					case 1:
						message.id = reader.readUint64()
						break
					case 2:
						message.name = reader.readString()
						break
					default:
						reader.skipField()
				}
			}
			return message
		}
		serializeBinary(): Uint8Array {
			return this.serialize()
		}
		static deserializeBinary(bytes: Uint8Array): updateName {
			return updateName.deserialize(bytes)
		}
	}
	export class updatePhone extends pb_1.Message {
		#one_of_decls: number[][] = []
		constructor(
			data?:
				| any[]
				| {
						id?: number
						phone?: string
				  }
		) {
			super()
			pb_1.Message.initialize(this, Array.isArray(data) ? data : [], 0, -1, [], this.#one_of_decls)
			if (!Array.isArray(data) && typeof data == 'object') {
				if ('id' in data && data.id != undefined) {
					this.id = data.id
				}
				if ('phone' in data && data.phone != undefined) {
					this.phone = data.phone
				}
			}
		}
		get id() {
			return pb_1.Message.getFieldWithDefault(this, 1, 0) as number
		}
		set id(value: number) {
			pb_1.Message.setField(this, 1, value)
		}
		get phone() {
			return pb_1.Message.getFieldWithDefault(this, 2, '') as string
		}
		set phone(value: string) {
			pb_1.Message.setField(this, 2, value)
		}
		static fromObject(data: { id?: number; phone?: string }): updatePhone {
			const message = new updatePhone({})
			if (data.id != null) {
				message.id = data.id
			}
			if (data.phone != null) {
				message.phone = data.phone
			}
			return message
		}
		toObject() {
			const data: {
				id?: number
				phone?: string
			} = {}
			if (this.id != null) {
				data.id = this.id
			}
			if (this.phone != null) {
				data.phone = this.phone
			}
			return data
		}
		serialize(): Uint8Array
		serialize(w: pb_1.BinaryWriter): void
		serialize(w?: pb_1.BinaryWriter): Uint8Array | void {
			const writer = w || new pb_1.BinaryWriter()
			if (this.id != 0) writer.writeUint64(1, this.id)
			if (this.phone.length) writer.writeString(2, this.phone)
			if (!w) return writer.getResultBuffer()
		}
		static deserialize(bytes: Uint8Array | pb_1.BinaryReader): updatePhone {
			const reader = bytes instanceof pb_1.BinaryReader ? bytes : new pb_1.BinaryReader(bytes),
				message = new updatePhone()
			while (reader.nextField()) {
				if (reader.isEndGroup()) break
				switch (reader.getFieldNumber()) {
					case 1:
						message.id = reader.readUint64()
						break
					case 2:
						message.phone = reader.readString()
						break
					default:
						reader.skipField()
				}
			}
			return message
		}
		serializeBinary(): Uint8Array {
			return this.serialize()
		}
		static deserializeBinary(bytes: Uint8Array): updatePhone {
			return updatePhone.deserialize(bytes)
		}
	}
	export class updateGender extends pb_1.Message {
		#one_of_decls: number[][] = []
		constructor(
			data?:
				| any[]
				| {
						id?: number
						gender?: boolean
				  }
		) {
			super()
			pb_1.Message.initialize(this, Array.isArray(data) ? data : [], 0, -1, [], this.#one_of_decls)
			if (!Array.isArray(data) && typeof data == 'object') {
				if ('id' in data && data.id != undefined) {
					this.id = data.id
				}
				if ('gender' in data && data.gender != undefined) {
					this.gender = data.gender
				}
			}
		}
		get id() {
			return pb_1.Message.getFieldWithDefault(this, 1, 0) as number
		}
		set id(value: number) {
			pb_1.Message.setField(this, 1, value)
		}
		get gender() {
			return pb_1.Message.getFieldWithDefault(this, 2, false) as boolean
		}
		set gender(value: boolean) {
			pb_1.Message.setField(this, 2, value)
		}
		static fromObject(data: { id?: number; gender?: boolean }): updateGender {
			const message = new updateGender({})
			if (data.id != null) {
				message.id = data.id
			}
			if (data.gender != null) {
				message.gender = data.gender
			}
			return message
		}
		toObject() {
			const data: {
				id?: number
				gender?: boolean
			} = {}
			if (this.id != null) {
				data.id = this.id
			}
			if (this.gender != null) {
				data.gender = this.gender
			}
			return data
		}
		serialize(): Uint8Array
		serialize(w: pb_1.BinaryWriter): void
		serialize(w?: pb_1.BinaryWriter): Uint8Array | void {
			const writer = w || new pb_1.BinaryWriter()
			if (this.id != 0) writer.writeUint64(1, this.id)
			if (this.gender != false) writer.writeBool(2, this.gender)
			if (!w) return writer.getResultBuffer()
		}
		static deserialize(bytes: Uint8Array | pb_1.BinaryReader): updateGender {
			const reader = bytes instanceof pb_1.BinaryReader ? bytes : new pb_1.BinaryReader(bytes),
				message = new updateGender()
			while (reader.nextField()) {
				if (reader.isEndGroup()) break
				switch (reader.getFieldNumber()) {
					case 1:
						message.id = reader.readUint64()
						break
					case 2:
						message.gender = reader.readBool()
						break
					default:
						reader.skipField()
				}
			}
			return message
		}
		serializeBinary(): Uint8Array {
			return this.serialize()
		}
		static deserializeBinary(bytes: Uint8Array): updateGender {
			return updateGender.deserialize(bytes)
		}
	}
}
